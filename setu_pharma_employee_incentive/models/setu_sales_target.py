...
class PharmaSalesTarget(models.Model):
    ...
    fiscal_period = fields.Selection(
        selection=[('monthly', 'Monthly'), ('quarterly', ' Quarterly'), ('quarterly+1', 'Quarterly+1'),
                   ('yearly', 'Yearly')], string='Fiscal Period', help="To Select Incentive Periods")
    sales_based_on = fields.Selection(
        selection=[('secondary_sales', 'Secondary Sales'), ('primary_sales', 'Primary Sales')])
    ...
    target_product_lines_ids = fields.One2many('setu.pharma.target.product.lines', 'target_id',
                                               help="Focused Products For Target")
    reward_type = fields.Selection(
        selection=[('fixed_amount', 'Fixed Amount'), ('based_on_percentage', 'Based On Percentage'),
                   ('international_trip', 'International Trip'), ('domestic_trip', 'Domestic Trip'),
                   ('international_domestic_trip', 'International+Domestic Trip'),
                   ('based_on_range', 'Based On Range')], help="Select Reward Type To Give Rewards To Employees")
    fiscal_month_ids = fields.Many2many('setu.pharma.fiscalperiod', string="Fiscal Months",
                                        help="Select Months To Give Rewards ")
    range_ids = fields.One2many('setu.sales.range', 'target_id', help="Sales Range For Incentive")
    ...
    status = fields.Selection([('draft', 'Draft'), ('running', 'Running'), ('expired', 'Expired')], readonly="True",
                              default='draft')
    ...
    def _compute_target_date(self):
        ...
        for record in self:
            if record.fiscal_month_ids:
                record.target_start_date = record.fiscal_month_ids.search([('name', '=', min(sorted(
                    record.fiscal_month_ids.mapped('name'), key=lambda m: datetime.strptime(m, "%B"))))]).start_date
                record.target_end_date = record.fiscal_month_ids.search([('name', '=', max(sorted(
                    record.fiscal_month_ids.mapped('name'), key=lambda m: datetime.strptime(m, "%B"))))]).end_date
    ...
    def _compute_message(self):
        ...
        for record in self:
            ...
            if record.based_on_sales and not record.reward_type == 'based_on_range':
                record.message = "If You Achieve The Target Of Amount <b><font color='#1f1f1f'>" + str(record.target_amount) + "</font></b> In <b><font color='#1f1f1f'>" + months + \
                             "</font></b>.<br> You Will Get <b><font color='#1f1f1f' >" + incentive + "</font></b>."
            elif record.reward_type == 'based_on_range' and record.based_on_sales:
                message = ''
                for range in record.range_ids:
                    message += "If You Achieve Target Between <b><font color='#1f1f1f'>" + str(range.range_from) + "</font></b> To <b><font color='#1f1f1f'>" + str(range.range_to)+ \
                             "</font></b>.<br> You Will Get <b><font color='#1f1f1f'>" + str(range.incentive_percentage) + "</font></b>% Incentive.<br>"
