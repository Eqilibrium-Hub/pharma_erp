...
class StockistMonthlyStatement(models.Model):
    ...
    previous_month_total_sales_amount = fields.Float("Previous Sales Amount", compute="_compute_value")
    closing_month_total_sales_amount = fields.Float("Closing Sales Amount", compute="_compute_amount")
    ...
    def check_fiscal_period(self):
        for record in self:
            fiscal_period = self.env['setu.stockist.monthly.statement'].search(
                [('partner_id', '=', self.env.context.get('active_id')), ('id', '!=', record.id)]).mapped('fiscal_period_id').mapped('id')
    ...
    def default_get(self, vals):
        ...
        fiscal_period = self.env['setu.stockist.monthly.statement'].search(
            [('partner_id', '=', self.env.context.get('active_id'))]).mapped('fiscal_period_id').mapped('name')
    ...
    def _compute_value(self):
        ...
        for record in self._origin:
            statement = self.env['setu.stockist.monthly.statement'].search([('partner_id', '=', record.partner_id.id),
    ...
    def _compute_amount(self):
        for record in self:
            ...
            record.next_month_order_total = sum(record.stockist_monthly_statement_ids.mapped('next_month_order_value'))
    ...
    def sale_order_creation(self):
        ...
        sale_order = False
        for record in self.stockist_monthly_statement_ids.filtered(lambda x: x.next_month_order_qty):
            if record and not sale_order:
                sale_order = self.env['sale.order'].create({
                    'partner_id': self.partner_id.id,
                    'date_order': record.stockist_monthly_statement_id.fiscal_period_id.start_date + relativedelta.relativedelta(months=1)
                })
    ...
    def action_view_sale_order(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("setu_pharma_basic.action_setu_sale_order")
        action['domain'] = [('id', '=', sale_id)]
        return action

    def _compute_difference(self):
        for record in self:
            sale_order = sum(self.env['sale.order'].search([('id','=', record.sale_order_id),('state','=','sale')]).order_line.mapped('price_subtotal'))
